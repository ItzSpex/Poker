...
namespace PokerDL.Mapping
{
    ...
    {
        protected string connectionString = @"Data Source=.;Initial Catalog=PokerDB;Integrated Security=True";
    ...
    public abstract class BaseDB<T> : BaseDB where T : BaseEntity, new()
    {
        ...
        {
            BaseIdentityEntity be = model as BaseIdentityEntity;
            command.CommandText = GetSQLInsertString();
            SetSQLParameters(model);
            try
            {
